var IncrementalDOM = require('incremental-dom'),
    ...
    notifications = IncrementalDOM.notifications,
...
var TodoView = Backbone.View.extend({
    tagName: 'div',
    className: 'kk',
    template: _.template("<h3>todo <%= what %></h3>"),
    container: null,
    title: null,
    todolist: [],
    ...
    initialize: function(options){
        ...
        var s1 = [ 'type', 'text', 'placeholder', '...', 'name', 'new-item', 'id', 'form_input_'+options.title];
        options.root.append(this.$el);
        patch(this.el, function() {
            elementOpen('h1', null, '', 'class', 'center');
                text(options.title);
            elementClose('h1');
            elementOpen('div', null, null, 'class', 'row');
                elementOpen('form', null, null, 'class', 'col s12', 'id', 'addtodo');
                    elementOpen('div', null, null, 'class', 'input-field col s12');
                        elementVoid('input', null, s1);  
                    elementClose('div');
                elementClose('form');
            elementClose('div');

            elementOpen('div', null, null, 'class', 'row');
                    elementOpen('div', null, null, 'class', 'col s12');
                        elementOpen('div', null, null, 'id', 'todo');
                            elementOpen('ul', 'key', null, 'class', 'collection', 'id', options.title);
                            elementClose('ul');
                        elementClose('div');
                    elementClose('div');
            elementClose('div');
    ...
    removefromlist: function(event){
        var key = $(event.target)[0].__incrementalDOMData.key;
        if (key != null) {
    ...
    updateListItem: function(container, arr){
        ...
        patch(container, function() {
            var count = 0;
            for(k in arr){
                var s2 = [ 'class', 'collection-item'];
                elementOpen('li', null, s2);
                    elementOpen('div', '', null);
                        text(arr[k]);
                        elementOpen('a', null, null, 'class', 'secondary-content', 'id', that.title+'-'+count, 'href', '#!');
                            elementOpen('i', k, null, 'class', 'material-icons');
                                text('send');
                            elementClose('i');
                        elementClose('a');
                    elementClose('div');
